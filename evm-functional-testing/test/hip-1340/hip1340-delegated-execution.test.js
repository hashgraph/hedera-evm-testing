...
describe("HIP-1340 - EIP-7702 Ethereum Specific tests - delegated execution", function () {
  before(async function () {
    ...
    log(
      "Starting test suite `%s` on network `%s` (chain id %s)",
  ...
  tests.forEach(({ trigger, authSenderTrigger }) => {
    it(`should getStorage and logs when a delegated EOA is the target of a transaction from \`${trigger}\` ${authSenderTrigger}`, async function () {
      const value = 42;
      ...
      const smartWallet = await deploy(
        "contracts/hip-1340/CustomSimple7702Account",
      );
      const eoa = await this.testCtx.createAndFundEOA();

      const [fromNonce, eoaNonce] = [new Nonce(), new Nonce()];
      const [authSender, authNonce] =
        authSenderTrigger === "SELF" ? [eoa, eoaNonce] : [from, fromNonce];
      await authSender
        .sendTransaction({
          type: 4,
      ...
      const storeAndEmitCall = encodeFunctionData(
        "storeAndEmit(uint256 value)",
        [value],
      );
      const data = encodeFunctionData(
        "execute(address target, uint256 value, bytes calldata data)",
        [storeAndEmit.address, 0, storeAndEmitCall],
      );

      const delegatedExecutionSender = trigger === "SELF" ? eoa : from;
      const delegatedExecutionSenderNonce =
        trigger === "SELF" ? eoaNonce : fromNonce;
      const receipt = await delegatedExecutionSender
        .sendTransaction({
          chainId: this.network.chainId,
          to: eoa.address,
      ...
      expect(receipt.logs[0]).to.deep.include({
        ...
        topics: [ethers.id("StoreAndEmitEvent(uint256)"), asHexUint256(value)],
      });

      const valueSlot = 0;
      const storedValue = await this.provider.getStorage(
        storeAndEmit.address,
        valueSlot,
      );
  ...
  it(`should getStorage with EOA address when delegated contract changes EOA storage`, async function () {
    ...
    // set delegation and update storage of the EOA
    const value = 41;
    const data = encodeFunctionData("storeAndEmit(uint256 value)", [value]);
    await eoa
      .sendTransaction({
        type: 4,
        chainId: this.network.chainId,
      ...
      .then((tx) => tx.wait());
    await verifyDelegation(eoa.address, storeAndEmit.address);
    // check storage slot
    const storedValue0 = await this.provider.getStorage(eoa.address, 0);
    expect(storedValue0).to.be.equal(asHexUint256(value));
  });

  it("should getStorage and logs when delegate execution to EOA in the same type 4 transaction", async function () {
    const value = 42;
    const storeAndEmit = await deploy("contracts/hip-1340/StoreAndEmit");
    const smartWallet = await deploy(
      "contracts/hip-1340/CustomSimple7702Account",
    ...
    const storeAndEmitCall = encodeFunctionData("storeAndEmit(uint256 value)", [
      value,
    ]);
    const data = encodeFunctionData(
      "execute(address target, uint256 value, bytes calldata data)",
  ...
  it("should log `msg.sender` and `tx.origin` with code length and hashes from an inner contract call", async function () {
    const senderAndOrigin = await deploy(
      "contracts/hip-1340/SenderAndOrigin",
      [],
      undefined,
      400_000,
    );
    const sender = await this.testCtx.authorizeEOADelegation(
      await this.testCtx.createAndFundEOA(),
      senderAndOrigin.address,
    );
    ...
    const designator = delegationIndicatorFor(
      senderAndOrigin.address.toLowerCase(),
    );
  ...
  it("should log the result of `codesize` and `codecopy`", async function () {
    const codeSizeAndCopy = await deploy(
      "contracts/hip-1340/CodeSizeAndCopy",
      [],
      undefined,
      300_000,
    );
    ...
    const designator = delegationIndicatorFor(
      codeSizeAndCopy.address.toLowerCase(),
    );
