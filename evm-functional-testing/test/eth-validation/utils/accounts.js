const { expect } = require('chai');
const {
    AccountUpdateTransaction,
    EthereumTransaction,
    EthereumTransactionData,
    Hbar,
    PrivateKey,
    PrecheckStatusError,
    ReceiptStatusError,
} = require('@hiero-ledger/sdk');
const { gas, txFees, isEthNetwork } = require('../../hip-1340/utils/web3');
const { MirrorNode } = require('evm-functional-testing/mirror-node');
const Async = require('../../../utils/async');

async function rotateAccountKey(accountId, oldPrivateKey, client) {
    const newPrivateKey = PrivateKey.generateECDSA();
    const tx = new AccountUpdateTransaction()
        .setAccountId(accountId)
        .setKey(newPrivateKey.publicKey)
        .freezeWith(client);
    const signed = await (await tx.sign(oldPrivateKey)).sign(newPrivateKey);
    await (await signed.execute(client)).getReceipt(client);
    return newPrivateKey;
}

async function buildEthTxRequest(wallet, toAddress, value, network) {
    return wallet.populateTransaction({
        type: 2,
        chainId: network.chainId,
        gasLimit: gas.base + gas.accountCreationCost(),
        to: toAddress,
        value,
        ...txFees(),
    });
}

/**
 * @returns {Promise<{ path: 'relay', mined: boolean, receiptStatus?: number, mirrorResult?: string, errorMessage?: string }>}
 */
async function submitEthTxViaRelay(wallet, toAddress, value, network) {
    const txRequest = await buildEthTxRequest(wallet, toAddress, value, network);

    let response;
    try {
        response = await wallet.sendTransaction(txRequest);
        if (isEthNetwork()) {
            const receipt = await response.wait();
            return { path: 'relay', mined: true, receiptStatus: receipt.status };
        }
    } catch (err) {
        return { path: 'relay', mined: false, errorMessage: err?.message ?? String(err) };
    }

    const result = await waitForMirrorContractResult(response.hash);
    return { path: 'relay', mined: true, mirrorResult: result.result };
}

/**
 * Polls the mirror node until the contract result for the given transaction hash is
 * ingested, then returns it. Uses the listing endpoint because the by-hash endpoint
 * responds "Not found" for failed EthereumTransactions.
 */
async function waitForMirrorContractResult(transactionHash) {
    const mirrorNode = new MirrorNode();
    const listing = await Async.waitForCondition(
        `mirror contract result for ${transactionHash}`,
        () => mirrorNode.getLatestContractResults(25),
        (r) => Array.isArray(r?.results) && r.results.some(x => x.hash === transactionHash),
        1000,
        15
    );
    return listing.results.find(x => x.hash === transactionHash);
}

/**
 * Polls the mirror node until it knows the given account. Submitting through the
 * relay before that makes the relay reject with "resource not found" without the
 * transaction ever reaching consensus, so the stale-key check would not be exercised.
 */
async function waitForMirrorAccount(address) {
    await Async.waitForCondition(
        `mirror account ${address}`,
        () => new MirrorNode().getAccount(address),
        (r) => r?.account != null,
        1000,
        15
    );
}

/**
 * @returns {Promise<{ path: 'sdk', status: string }>}
 */
async function submitEthTxViaSdk(wallet, toAddress, value, network, client) {
    const txRequest = await buildEthTxRequest(wallet, toAddress, value, network);
    const signedTx = await wallet.signTransaction(txRequest);
    const ethData = EthereumTransactionData.fromBytes(
        Buffer.from(signedTx.slice(2), 'hex')
    );

    let response;
    try {
        response = await new EthereumTransaction()
            .setEthereumData(ethData.toBytes())
            .setMaxGasAllowanceHbar(new Hbar(2))
            .execute(client);
    } catch (err) {
        if (err instanceof PrecheckStatusError) {
            return { path: 'sdk', status: err.status.toString() };
        }
        throw err;
    }

    try {
        const receipt = await response.getReceipt(client);
        return { path: 'sdk', status: receipt.status.toString() };
    } catch (err) {
        if (err instanceof ReceiptStatusError) {
            return { path: 'sdk', status: err.status.toString() };
        }
        throw err;
    }
}

function isRelayRejection(result) {
    if (result.mined) {
        return result.receiptStatus === 0 || result.mirrorResult === 'INVALID_SIGNATURE';
    }
    const message = result.errorMessage ?? '';
    return (
        message.includes('INVALID_SIGNATURE') ||
        message.includes('Requested resource not found')
    );
}

async function expectRelayEthTxHbarTransferToFail(wallet, toAddress, value, network) {
    const result = await submitEthTxViaRelay(wallet, toAddress, value, network);
    expect(
        isRelayRejection(result),
        `relay should reject the EthTx (mined=${result.mined}, mirrorResult=${result.mirrorResult ?? 'n/a'}, error=${result.errorMessage ?? 'none'})`
    ).to.be.true;
}

async function expectSdkEthTxHbarTransferToFail(
    wallet,
    toAddress,
    value,
    network,
    client,
    expectedStatus = 'INVALID_SIGNATURE'
) {
    const result = await submitEthTxViaSdk(wallet, toAddress, value, network, client);
    expect(
        result.status,
        'SDK EthereumTransaction should reject the EthTx'
    ).to.equal(expectedStatus);
}

async function expectEthTxHbarTransferToFailViaBothPaths(
    wallet,
    toAddress,
    value,
    network,
    client
) {
    if (!isEthNetwork()) {
        await waitForMirrorAccount(wallet.address);
    }
    await expectRelayEthTxHbarTransferToFail(wallet, toAddress, value, network);
    await expectSdkEthTxHbarTransferToFail(
        wallet,
        toAddress,
        value,
        network,
        client
    );
}

module.exports = {
    rotateAccountKey,
    buildEthTxRequest,
    submitEthTxViaRelay,
    submitEthTxViaSdk,
    expectRelayEthTxHbarTransferToFail,
    expectSdkEthTxHbarTransferToFail,
    expectEthTxHbarTransferToFailViaBothPaths,
};
