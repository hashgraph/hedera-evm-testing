...
describe("HIP-1215 System Contract testing. scheduleCall()", () => {
  ...
  describe("HSS schedule delegation designator", function () {
    it("should return delegation designation to schedule address when an HSS schedule is created", async function () {
      const scheduleTx = await hip1215.scheduleCall(
        ...
        GAS_LIMIT_1_000_000.gasLimit + gasIncrement++,
        0,
        addTestCallData("schedule delegation designator"),
      );
      const scheduleAddress = await expectScheduleCallEvent(
        scheduleTx,
        ResponseCodeEnum.SUCCESS.valueOf(),
      );
      const code = await ethers.provider.getCode(scheduleAddress);
      expect(code).to.be.equal(delegationIndicatorFor(HSS_ADDRESS));
    });
  });
