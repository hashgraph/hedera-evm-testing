...
- **`SELFDESTRUCT` semantics** — Hedera entities are not removed the way Ethereum accounts are, so redeploying to the same `CREATE2` address after `SELFDESTRUCT` is not testable for parity (and is unstable on Ethereum itself post-Cancun, per EIP-6780)
